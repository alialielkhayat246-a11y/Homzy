...
export function installSalesWorkspace({session, reload}) {
  ...
  function renderTab(){
    ...
    content().innerHTML=card(tr('ملخص العميل','Client summary'),`<p>${esc(state.summary)}</p>`)+`<div class="sales-grid">`+card(tr('درجة اهتمام استرشادية','Indicative intent score'),`<div class="sales-score">${s.score}/100</div><p>${esc(label(s.temperature))}</p><ul>${s.signals.map(x=>`<li>${esc(label(x.signal))} +${x.points}${x.count?' ('+x.count+')':''}</li>`).join('')}</ul><p class="sales-muted">${tr('آخر ٣٠ يومًا من النشاط المسجّل. ليست نسبة احتمال إتمام الصفقة.','Last 30 days of recorded activity. This is not a closing probability.')}</p>`)+card(tr('الخطوة التالية','Next best action'),`<p>${esc(state.next_best_action)}</p><p>${esc(state.followup_recommendation)}</p><p>${state.missing_information.map(label).map(esc).join(' · ')}</p>${state.risks.map(r=>`<p>${esc(r)}</p>`).join('')}`)+`</div><p class="sales-muted">${tr('توصيات مبنية على قواعد قابلة للتطوير وبيانات حقيقية.','Recommendations use configurable rules and recorded facts.')}</p>`;
  }
  ...
  function renderMatches(){
    content().innerHTML=`<p>${tr('اختار حتى ٨ وحدات لإنشاء عرض موثق. البيانات الناقصة تقلل درجة المطابقة.','Select up to 8 properties for a recorded offer. Missing inventory details reduce the match score.')}</p>${state.inventory_limited?'<p class="sales-error">'+tr('النتائج محدودة بأول ١٠ آلاف وحدة','Results limited to the first 10,000 listings')+'</p>':''}`+state.matches.map((m,i)=>card(m.property.title||'Homzy',`<label><input type="checkbox" data-match="${i}"> ${tr('إضافة للعرض','Add to offer')}</label><div class="sales-score">${m.score}%</div><p>${esc(m.property.area||'—')} · ${esc(fmt(m.property.price))} ${esc(m.property.currency||'EGP')}</p><p class="sales-muted">${tr('اكتمال بيانات المقارنة','Comparison data coverage')}: ${m.coverage}%</p><ul class="sales-evidence">${m.evidence.map(x=>`<li class="${x.status}">${esc(label(x.criterion))}: ${x.status==='matched'?tr('مطابق','Matches'):x.status==='unknown'?tr('بيانات ناقصة','Missing data'):tr('غير مطابق','Does not match')} <small>${esc(fmt(x.actual))}</small></li>`).join('')}</ul>`)).join('')+`<button id="sales-offer" class="sales-primary">${tr('إنشاء عرض ومعاينته','Create and preview offer')}</button>`;
    dialog.querySelector('#sales-offer').onclick=e=>run(e.target,async()=>{
